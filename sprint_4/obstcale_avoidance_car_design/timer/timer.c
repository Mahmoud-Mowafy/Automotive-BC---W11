...
void timer_init(const TimerConfig_t *Config)
{
    ...
    for(i=0; i< MAX_TIMERS; i++)
    {
        ...
        // Loop through the configuration table and set each register
         // choose timer mode
        if(Config[i].TimerEnable == TMR_ENABLED)
        {

            if( Config[i].TimerMode == TMR_OVERFLOW_MODE )
            {
                /* select the normal mode for the timer, timer is not start yet.*/
               /* if(Config[i].TimerChannel == TIMER0)
                {
                    EN_TMR_ERROR_T TMR_tmr0NormalModeInit(Config[i].ISREnable);
                }*/
                if(Config[i].TimerChannel == TIMER1)
                {
                    TIMER_tmr1NormalModeInit(Config[i].ISREnable);
                }
                if(Config[i].TimerChannel == TIMER2)
                {
